...
- signaling 부하에서 누적 드롭 17~45%다(발열로 런별 변동).
- 드롭은 전부 커널 캡처 링버퍼(ps_drop)에서 나고 NIC 드롭(ps_ifdrop)은 0이다.
- 링버퍼를 키워도 드롭이 지속된다. 2MB 21.6%, 16MB 35.8%, 64MB 32.2%.
- 드롭 구간에 cpu_all이 60~68%(약 1.5코어 유휴), RAM이 약 1.9GB 여유고 OOM·스왑이 없다.
- 패킷당 처리시간은 약 112µs, 처리율 천장은 약 9,000 pps다.
- 캡처 스레드 시간 구성은 파싱+그룹화 90%, 락 대기 8.2%, IP 재조립 1.7%, dump 0.1%다.
- 파싱+그룹화 내부는 파싱 72.7%, 그룹화 14.8%다.
- RTP는 100cps만으로 약 42k pps이고, 500cps부터 누적 드롭 45%까지 오르며 전 코어가 약 90%
  이상 포화된다.
