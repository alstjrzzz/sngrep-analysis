...
파싱, 그룹화, 표시는 전역 뮤텍스(capture_cfg.lock) 하나로 직렬화되고 UI 렌더 스레드도 같은
락을 잡기 때문에, 화면을 갱신하는 동안에는 파싱이 진행되지 못한다.
...
| 위치 | 종류 | 내용 |
|---|---|---|
| src/capture.c | 드롭 측정 | 드롭 모니터 스레드. 온라인 소스마다 pcap_stats()를 주기적으로 샘플링해 CSV로 기록한다. 캡처 락과 파싱 경로를 건드리지 않아 측정을 왜곡하지 않고, parse_packet이 멈춰도 계속 샘플링한다. 환경변수 SNGREP_STATS_CSV, SNGREP_STATS_INTERVAL_MS로 제어한다. |
| src/capture.c | 단계별 시간 측정 | parse_packet 단계별 타이머(IP 재조립, TCP 재조립, 락 대기, 파싱+그룹화, 덤프)를 누적해 profile.csv로 기록한다. SNGREP_PROFILE이 설정됐을 때만 켜져서 드롭 측정에는 부담을 주지 않는다. |
| src/capture.h | 단계별 시간 측정 | sip.c와 공유하는 프로파일링 전역 변수와 prof_now_ns() 선언. |
| src/sip.c | 단계별 시간 측정 | sip_check_packet 안에서 파싱(정규식, 페이로드 파싱)과 그룹화(Call-ID 조회, 메시지 추가, 상태 갱신) 시간을 따로 잰다. |
| bench/ | 측정 자동화 | sngrep(실제 TUI를 tmux에서 구동), SIPp UAS, 코어별 CPU/RAM 샘플러, SIPp 콜레이트 램프를 한 번에 돌리고 결과를 타임스탬프 폴더에 모아 matplotlib 그래프까지 만든다. 자세한 사용법은 bench/README.md 참고. |
...
- T1, 드롭이 실제로 나는가, 어디서 나는가: signaling 램프, 기본 -B 2. ps_drop과 ps_ifdrop을
