...
## 7. 현재까지 결과 → **[RESULTS.md](RESULTS.md)**

전체 측정 결과 리포트는 **[RESULTS.md](RESULTS.md)** 참고. 요약:

- **T1**: sngrep은 부하에서 드롭한다. 전부 `ps_drop`(캡처 링버퍼), `ps_ifdrop`=0.
  버퍼를 64MB로 키워도 32% 드롭. 코어 유휴·RAM 여유인데도 드롭 → **단일 스레드 한계**.
- **T4**: 캡처 스레드 시간의 **90%가 파싱+그룹화** (패킷당 ~104µs) → 파싱 병렬화 정당화.
