...
- **SIP 파싱이 비싼 경로다.** 패킷당 약 22µs(regex)로 RTP(약 3µs)의 7~18배다. 캡처+파싱은
  단일 스레드라 SIP에서 코어 하나가 천장이다(이 호스트 ≈ 45k pps).
- **RTP 드롭은 sngrep이 아니라 발신측 SIPp 경합 아티팩트였다.** sngrep을 자기 코어에 격리하면
  같은 RTP 볼륨을 거의 안 떨군다(0.007%).
- **절대 드롭률은 이 노트북 환경에서 재현되지 않는다.** VM이 호스트 발열을 격리하지 못하고,
  드롭이 용량 절벽 + 재전송 피드백 위에서 나서 같은 부하에도 0~수십%로 양극화된다. 그래서
  드롭률은 지표로 쓰지 않는다.

그래프, 근거, 수치는 [RESULTS.md](RESULTS.md)에 있다.
