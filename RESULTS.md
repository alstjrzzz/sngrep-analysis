...
![rtp 비격리](bench/results/20260630_154908_rtp_B2/report_pps.png)

비격리. 받은 RTP(파랑) 중 일부가 드롭(빨강)된다.

![rtp 격리](bench/results/20260630_164644_rtp_B2/report_pps.png)

격리(sngrep 코어 0 전용). 받은 pps가 200k까지 가는데도 드롭(빨강)은 바닥이다.
...
SIPp가 CPU를 많이 쓰는 것 자체는 정상이다. 문제는 부하 생성기(SIPp)와 측정 대상(sngrep)을 같은
VM 한 대에 태워, 실제 운영에는 없는 CPU 경쟁을 만든 것이다. taskset으로 코어를 나눈 건 SIPp를
고친 게 아니라 이 경쟁을 없앤 것이다.

이 문제와 해법은 부하 테스트 일반에서 잘 알려져 있다. k6, JMeter, Gatling 같은 도구의 문서는
공통적으로 부하 생성기와 대상을 같은 머신에 두지 말라고 명시한다. 생성기 자신도 CPU·네트워크를
많이 써서 대상의 자원을 뺏거나 생성기 자체가 먼저 포화돼 부하를 충분히 못 내는 경우가 흔하기
때문이다. 표준 구성은 생성기와 대상을 처음부터 별도 머신(또는 별도 노드)에 두는 것이고, 이렇게
분리하면 실행마다 격리 작업을 반복할 필요가 없다. 이번 taskset은 머신이 한 대뿐인 랩 환경에서
그 분리를 코어 단위로 흉내낸 것이고, proc.csv로 생성기 자신의 CPU를 같이 측정한 것도 같은
이유다(생성기가 병목인지 대상이 병목인지를 가려야 하므로).
...
![rtp 코어별 CPU](bench/results/20260630_154908_rtp_B2/report_cores.png)

RTP 비격리 런의 코어별 CPU. 여러 코어가 뜨거운 건 sngrep이 아니라 SIPp 미디어 스레드들이다.
