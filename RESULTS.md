...
코어가 놀고 메모리도 남는데 드롭하므로, 총 CPU 부족이 아니라 단일 스레드 한계다. sngrep의
캡처+파싱 스레드 하나가 코어 하나를 못 넘고 남는 코어를 쓰지 못한다.

cpu_all은 시스템 전체라 SIPp도 포함하지만 결론은 그 분해와 무관하다. 전체가 60~68%면
sngrep과 SIPp가 어떻게 나뉘든 코어 여유가 있다는 뜻이고, 스레드가 코어 하나를 포화시킨다는
것은 위 T4 프로파일러가 sngrep만 재서 직접 보여준다.
