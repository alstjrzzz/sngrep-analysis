...
각 단계가 캡처 스레드 시간에서 차지하는 비중과 패킷당 평균이다.

- 병목은 파싱+그룹화로 캡처 스레드 시간의 90%다. 그 중 SIP 파싱이 약 73%로 패킷마다 regex를
  돌리며 약 74µs/패킷이고, 그룹화는 약 15%로 약 15µs/패킷이다. 파싱은 패킷마다 독립적으로
  돌고, 그룹화는 Call-ID 맵을 공유하지만 비중이 작다. 나머지는 페이로드 복사·할당 등이다.
- 패킷당 약 112µs이므로 단일 스레드 처리율 천장은 약 9,000 pps다. 도착이 이를 넘으면
  링버퍼가 차서 드롭한다.
- 실측 평균 처리량도 이와 맞는다. signaling 런에서 sngrep이 드레인한 누적 패킷은 157만 개로
  평균 약 8,970 pps다. 평균 도착은 약 10,800 pps이고, 그 차이가 드롭으로 빠졌다.
